...
const MovieDetails = () => {
    ...
    const fetchDetails = async () => {
      try {
        const res = await axios.get(
          `http://www.omdbapi.com/?i=${id}&apikey=1e9eaaca`
        );
        setMovie(res.data);
  ...
  return (
   <div className="flex items-center justify-center w-full bg-gradient-to-br from-gray-900 via-gray-800 to-black p-4">
  <div className="grid max-w-3xl grid-cols-1 gap-6 md:grid-cols-2 bg-gray-800 bg-opacity-50 rounded-xl shadow-2xl overflow-hidden backdrop-filter backdrop-blur-lg border border-gray-700">
    ...
    <div className="p-6 text-amber-50">
      ...
      <div className="space-y-2 text-base">
        ...
        <p>
          <strong className="font-semibold text-amber-300">Director:</strong> 
          <span className="ml-2 text-gray-300">{movie.Director}</span>
        </p>
        <p>
          <strong className="font-semibold text-amber-300">IMDB Rating:</strong> 
